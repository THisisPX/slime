...
SAVE_DIR="${SAVE_DIR:-/workspace/volume/pengxiong/models/Qwen3-VL-4B_slime_geo3k_v2}"
...
ROLLOUT_ARGS=(
   ...
   # v2: rollout-batch-size 16 × n-samples 8 = 128 样本/rollout (与 v1 相同总样本量)
   #     但每个 prompt group 有 8 个样本 (v1 仅 4), 降低 zero-std 组比例
   --num-rollout 100
...
MISC_ARGS=(
   ...
   --use-tensorboard
   --tb-project-name qwen3-vl-4b-geo3k-4gpu-v2
)
...
# 日志目录
LOG_DIR="${LOG_DIR:-logs/qwen3-vl-4b-geo3k-4gpu-v2}"
mkdir -p "${LOG_DIR}"
